...
class CloudflareManager:
    ...
    def run(self):
        ...
        if total_lines > self.max_list_size * self.max_lists:
            error(
                f"The domains list has more than {self.max_list_size * self.max_lists} lines")
            return
        ...
        total_domains = sum(
            [l['count'] for l in current_lists_result]) if current_lists_result else 0
        ...
        if current_lists_count > 0:
            used_list_ids, excess_list_ids, missing_indices = utils.update_lists(
                current_lists, chunked_lists, self.adlist_name)

        if missing_indices or not used_list_ids:
            used_list_ids += utils.create_lists(chunked_lists,
                                                missing_indices, self.adlist_name)

        if not used_list_ids:
            used_list_ids = utils.create_lists(
                chunked_lists, range(1, total_lists + 1), self.adlist_name)

        utils.update_or_create_policy(
            current_policies, used_list_ids, self.policy_name)
